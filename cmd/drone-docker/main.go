...
func main() {
	...
	app.Flags = []cli.Flag{
		...
		cli.GenericFlag{
			Name:   "custom-labels-env-map",
			Usage:  "additional labels k=v, where v is a env var",
			EnvVar: "PLUGIN_CUSTOM_LABELS_ENV_MAP",
			Value:  &StringMapFlag{},
		},
		cli.StringSliceFlag{
			Name:   "label-schema",
...
func run(c *cli.Context) error {
	plugin := docker.Plugin{
		...
		Build: docker.Build{
			...
			LabelsEnvMap: c.Generic("custom-labels-env-map").(*StringMapFlag).Get(),
